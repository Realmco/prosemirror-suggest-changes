 ...
 */ export function suggestReplaceStep(trackedTransaction, state, doc, step, prevSteps, suggestionId, createExtraAttrs, preventJoin) {
    ...
    const extraAttrs = createExtraAttrs?.() ?? {};
    const useMarkBeforeId = markBefore && !preventJoin?.(markBefore.attrs, extraAttrs);
    const useMarkAfterId = markAfter && !preventJoin?.(markAfter.attrs, extraAttrs);
    const markId = useMarkBeforeId ? markBefore.attrs["id"] : useMarkAfterId ? markAfter.attrs["id"] : suggestionId;
        ...
        if ($stepFrom.parent !== $stepTo.parent) {
            ...
            if (blockRange) {
                ...
                while(d > minDepth){
                    ...
                    const stepFromBlockBoundarySuggestion = blockBoundarySuggestion.isInSet(trackedTransaction.doc.nodeAt(stepFromBlockStart)?.marks ?? [])?.attrs;
                    ...
                    if (!stepFromBlockBoundarySuggestion) {
                        ...
                        trackedTransaction.addNodeMark(stepFromBlockStart, blockBoundarySuggestion.create({
                            id: markId,
                            type: deletion.name,
                            ...extraAttrs
                        }));
                        ...
                        alreadyDeleted = false;
                        trackedTransaction.removeNodeMark(stepFromBlockStart, blockBoundarySuggestion);
                        trackedTransaction.join(stepFromBlockStart + // oxlint-disable-next-line typescript/no-non-null-assertion
    ...
    if (nodeAfter && markAfter && markAfter.attrs["id"] !== markId && !preventJoin?.(markAfter.attrs, {
        id: markId,
        ...extraAttrs
    })) {
        ...
        if ($insertFrom.parent !== $insertedTo.parent) {
            ...
            if (blockRange) {
                ...
                for(let d = $insertFrom.depth; d > blockRange.depth; d--){
                    ...
                    const insertFromBlockBoundarySuggestion = blockBoundarySuggestion.isInSet(trackedTransaction.doc.nodeAt(insertFromTextblock)?.marks ?? [])?.attrs;
                    trackedTransaction.addNodeMark(insertFromTextblock, blockBoundarySuggestion.create({
