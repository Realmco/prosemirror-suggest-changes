 ...
 * marks.
 */ export function transformToSuggestionTransaction(originalTransaction, state, generateId, extraAttrs, preventJoin) {
    ...
    for(let i = 0; i < originalTransaction.steps.length; i++){
        ...
        if (stepTracker(trackedTransaction, state, doc, step, originalTransaction.steps.slice(0, i), suggestionId, extraAttrs, preventJoin) && i < originalTransaction.steps.length - 1) {
 ...
 */ export function withSuggestChanges(dispatchTransaction, generateId, extraAttrs, preventJoin) {
    ...
    return function dispatchTransaction(tr) {
        ...
        const transaction = isSuggestChangesEnabled(this.state) && !tr.getMeta("history$") && !tr.getMeta("collab$") && !ySyncMeta.isUndoRedoOperation && !ySyncMeta.isChangeOrigin && !("skip" in (tr.getMeta(suggestChangesKey) ?? {})) ? transformToSuggestionTransaction(tr, this.state, generateId, extraAttrs, preventJoin) : tr;
