...
export function findSuggestionMarkEnd($pos, markType, crossBoundaries = false) {
    const { blockBoundarySuggestion } = getSuggestionMarks($pos.doc.type.schema);
    const initialMark = ($pos.nodeAfter ?? $pos.nodeBefore)?.marks.find((mark)=>mark.type === markType);
    if (!initialMark) {
        return $pos.pos;
    }
    ...
    while(true){
        ...
        const initialBoundarySuggestion = blockBoundarySuggestion.isInSet(parent.marks);
        if (!initialBoundarySuggestion || initialBoundarySuggestion.attrs["type"] !== markType.name) {
            return markEndPos;
        }
        let d = $pos.depth;
