 ...
 * trackReplaceStep().
 */ export function suggestReplaceAroundStep(trackedTransaction, state, doc, step, prevSteps, suggestionId, extraAttrs, preventJoin) {
    ...
    const replace = replaceStep(doc, step.getMap().invert().map(blockRange.start, -1), step.getMap().invert().map(blockRange.end, 1), applySuggestionsToRange(applied, blockRange.start, blockRange.end));
    if (!replace) return false;
    return suggestReplaceStep(trackedTransaction, state, doc, replace, prevSteps, suggestionId, extraAttrs, preventJoin);
}
